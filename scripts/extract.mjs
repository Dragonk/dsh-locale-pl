...
async function evalExpr(expr, ctx, depth = 0) {
  ...
  if (t[0] === "'" || t[0] === '"' || t[0] === '`') {
    // A value may be a concatenation chain ('a' + 'b' + CONST); split at
    // top-level '+' and join the pieces.
    const parts = splitTopLevel(t, '+')
    if (parts.length > 1) {
      let joined = ''
      for (const p of parts) joined += String(await evalExpr(p, ctx, depth + 1))
      return joined
    }
    return t[0] === '`' ? decodeTemplate(t) : readStringLiteral(t, 0).value
  }
...
function importModule(spec, importerFile) {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      // A dictionary module can fail to import when one of its dependencies is
      // not installed (for example a re-export of an unbuilt workspace package).
      // Its `zh`/`en` exports are plain literals, so parse them textually and
      // keep going instead of dropping the whole namespace.
      mod = await fallbackParseDictModule(resolved)
      if (Object.keys(mod).length === 0) throw err
      warn(`[${relToClone(resolved)}] imported through the literal fallback: ${err.message}`)
    }
...
async function fallbackParseDictModule(absPath) {
  ...
  for (const m of text.matchAll(/\bexport\s+const\s+(zh|en)\b/g)) {
    const name = m[1]
    // The declaration may carry a type annotation between the name and '='
    // (e.g. `export const en: Record<Key, string> = {`); find the assignment.
    let eqIdx = text.indexOf('=', m.index + m[0].length)
    while (eqIdx !== -1 && text[eqIdx + 1] === '>') eqIdx = text.indexOf('=', eqIdx + 1)
    if (eqIdx === -1) continue
    const eq = skipTrivia(text, eqIdx + 1)
    if (text[eq] !== '{') continue
